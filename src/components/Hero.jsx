...
export default function Hero() {
  ...
  return (
    <>
      {/* ── Fixed nav — persists through all sections ── */}
      <nav
      ...
      >
        {/* Logo dot */}
        <motion.div
          custom={0} variants={fadeDown} initial="initial" animate="animate"
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map(({ label, href }, i) => (
            <motion.a
              key={label} custom={i + 1} variants={fadeDown}
              initial="initial" animate="animate"
              href={href}
              className="text-sm font-semibold tracking-widest uppercase text-white hover:opacity-60 transition-opacity"
            >
              {label}
            </motion.a>
          ))}
      ...
      {menuOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[60] bg-white flex flex-col px-5 pt-5 pb-8"
        >
          ...
          <div className="mt-auto">
            <a
              ...
              style={{ color: RED }}
            >
              Nous Contacter <ArrowUpRight size={22} />
            </a>
          </div>
        </motion.div>
      )}
      ...
      >
        ...
        {/* Content — push down for fixed nav */}
        <div className="relative z-10 flex flex-col flex-1 pt-20">
          ...
          <div className="flex-1 flex items-center justify-end px-5 sm:px-8 md:px-12 py-8 md:py-0">
            <div className="flex items-end gap-5 sm:gap-8 md:gap-10">
              {stats.map((stat, i) => (
                <motion.div
                  key={stat.label} custom={i + 2} variants={fadeUp}
                  initial="initial" animate="animate"
